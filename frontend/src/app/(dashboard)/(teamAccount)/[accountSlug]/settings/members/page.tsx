...
export default async function TeamMembersPage({
  ...
  // Local Basejump migrations can expose either account_role or role.
  const teamAccountRole =
    (teamAccount as { account_role?: string } | null)?.account_role ??
    teamAccount?.role;

  if (!teamAccount || teamAccountRole !== 'owner') {
    return (
      <Alert
        variant="destructive"
        className="border-red-300 dark:border-red-800 rounded-xl"
