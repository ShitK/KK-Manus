...
const githubInterviewPrompt: PromptExample = {
  title: '面试准备',
  query:
    '这个 GitHub 仓库是我做的项目：<仓库 URL>\n请你做我的面试官，根据 README 和代码帮我生成几道面试题。\n请用中文展示，优先从架构设计和实现细节开始问。\n后面点评我的回答时，请既指出优点，也指出需要补充的地方。',
  icon: <Github className="text-sky-700 dark:text-sky-400" size={16} />,
};
...
const getDisplayedPrompts = (count: number = 3): PromptExample[] => [
  githubInterviewPrompt,
  ...getRandomPrompts(Math.max(count - 1, 0)),
];
...
export const Examples = ({
  ...
  useEffect(() => {
    setDisplayedPrompts(getDisplayedPrompts(count));
  }, [count]);
