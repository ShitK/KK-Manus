...
const toolResultToStatus = (toolResult: any): string | null => {
  if (!toolResult) return null;
  if (typeof toolResult === 'string') return toolResult;
  if (typeof toolResult.toolOutput === 'string') return toolResult.toolOutput;
...
const extractImagePathFromStatus = (status: string | null): string | null => {
  if (!status) return null;
  const imagePathMatch = status.match(
    /Image saved as:\s*([^\s.]+\.(png|jpg|jpeg|webp|gif))/i,
  );
  return imagePathMatch ? imagePathMatch[1] : null;
};
...
const extractFromLegacyFormat = (content: any): ImageEditGenerateData => {
  ...
  if (toolData.toolResult && toolData.arguments) {
    const status = toolResultToStatus(toolData.toolResult);
    const generatedImagePath = extractImagePathFromStatus(status);
    ...
    return {
      ...
      status,
      success:
        typeof toolData.toolResult === 'object'
          ? Boolean((toolData.toolResult as any).isSuccess)
          : undefined,
      timestamp: undefined,
      error:
        typeof toolData.toolResult === 'object' &&
        typeof (toolData.toolResult as any).error === 'string'
          ? (toolData.toolResult as any).error
          : null,
    };
  }
