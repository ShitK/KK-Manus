...
export const DEFAULT_FREE_MODEL_ID = 'deepseek/deepseek-v4-flash';
...
export const useModelSelection = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedModel && LEGACY_DEFAULT_MODEL_IDS.has(savedModel)) {
      ...
      const defaultModel = isLocalMode()
  ...
  const getActualModelId = (modelId: string): string => {
    if (LEGACY_DEFAULT_MODEL_IDS.has(modelId)) {
