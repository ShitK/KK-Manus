...
export interface ChatInputProps {
  ...
  /**
   * Optional caller-owned prompt template surface.
   * Callers should only pass it when the current context, input state, and
   * agent status make templates appropriate for the active workflow.
   */
  promptTemplateSlot?: React.ReactNode;
            ...
            <div className="w-full text-sm flex flex-col justify-between items-start rounded-lg">
              <CardContent className={`w-full p-1.5 pb-2 ${bgColor} border rounded-3xl`}>
                <AttachmentGroup
                  ...
                  showPreviews={true}
                />
                {!disabled && uploadedFiles.length === 0 ? promptTemplateSlot : null}
                <MessageInput
                  ref={textareaRef}
                  value={value}
