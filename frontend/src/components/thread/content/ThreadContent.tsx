...
const getVisibleAgentName = (name?: string | null) => name === 'Suna' ? 'KKManus' : name;
...
export const ThreadContent: React.FC<ThreadContentProps> = ({
    ...
    const getAgentInfo = useCallback(() => {
        ...
        if (agentData && !isSunaDefaultAgent) {
            ...
            const visibleAgentName = getVisibleAgentName(agentData.name || agentName) || 'KKManus';
            const avatar = profileUrl ? (
                <img src={profileUrl} alt={visibleAgentName} className="h-5 w-5 rounded object-cover" />
            ) : agentData.avatar ? (
                <div className="h-5 w-5 flex items-center justify-center rounded text-xs">
        ...
        if (recentAssistantWithAgent?.agents?.name) {
            ...
            const visibleAgentName = getVisibleAgentName(recentAssistantWithAgent.agents.name) || getVisibleAgentName(agentName) || 'KKManus';
        ...
        if (isSunaDefaultAgent) {
            return {
                name: getVisibleAgentName(agentName) || 'KKManus',
                avatar: (
                    <div className="h-5 w-5 flex items-center justify-center rounded text-xs">
                        <KortixLogo size={16} />
                    </div>
                    ...
                    <div ref={contentRef} className="mx-auto min-w-0 w-full max-w-3xl px-4 md:px-6">
                        <div className="space-y-8 min-w-0">
                            {(() => {
                                ...
                                const ensureLiveStreamingAssistantGroup = () => {
                                    const lastGroup = finalGroupedMessages.at(-1);
                                    ...
                                    finalGroupedMessages.push({
                                        type: 'assistant_group',
                                        messages: [],
                                        key: `assistant-group-${assistantGroupCounter}-streaming-shell`,
                                    });
                                };

                                // Live streaming text is rendered by the dedicated last-group block below.
                                // Create only the assistant group shell here; adding streaming text as a
                                // synthetic message duplicates the opening prose.
                                if (
                                    streamingTextContent &&
                                    !readOnly &&
                                    ...
                                    ensureLiveStreamingAssistantGroup();
                                }
