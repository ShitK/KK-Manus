...
async function rpc<TName extends BasejumpRpcName>(
  name: TName,
  args = {} as BasejumpRpcArgs<TName>,
): Promise<BasejumpRpcResponse<BasejumpRpcResult<TName>>> {
  const response = await fetch('/api/basejump/rpc', {
    method: 'POST',
  ...
  if (!response.ok) {
    return {
      data: null,
      error: result?.error ?? { message: 'Basejump RPC failed' },
    };
  }
...
type LocalQueryResult<TData = any> = {
  data: TData | null;
  error: any | null;
};

class LocalQueryBuilder<TData = any> implements PromiseLike<LocalQueryResult<TData>> {
  select(_columns = '*'): LocalQueryBuilder<TData> {
    return this;
  }

  eq(_column: string, _value: any): LocalQueryBuilder<TData> {
    return this;
  }

  in(_column: string, _values: any[]): LocalQueryBuilder<TData> {
    return this;
  }
  ...
  ): LocalQueryBuilder<TData> {
    return this;
  }

  limit(_count: number): LocalQueryBuilder<TData> {
    return this;
  }

  range(_from: number, _to: number): LocalQueryBuilder<TData> {
    return this;
  }

  single(): Promise<LocalQueryResult<TData>> {
    return Promise.resolve({ data: null, error: null });
  }

  maybeSingle(): Promise<LocalQueryResult<TData>> {
    return Promise.resolve({ data: null, error: null });
  }

  then<TResult1 = LocalQueryResult<TData>, TResult2 = never>(
    onfulfilled?:
      | ((value: LocalQueryResult<TData>) => TResult1 | PromiseLike<TResult1>)
      | null,
...
class LocalTableBuilder {
  select(columns = '*'): LocalQueryBuilder {
    return new LocalQueryBuilder().select(columns);
  }

  insert(_data: Record<string, any> | Record<string, any>[]): LocalQueryBuilder {
    return new LocalQueryBuilder();
  }

  update(_data: Record<string, any>): LocalQueryBuilder {
    return new LocalQueryBuilder();
  }
...
export function createClient() {
  return {
    ...
    rpc,
    // 添加数据库相关方法以兼容现有代码
    from: (_table: string) => new LocalTableBuilder(),
    // 添加存储相关方法
    storage: {
      from: () => ({
