...
export type GitHubRepoInterviewMemoryPrivacy = {
  includes_user_answer: boolean;
  includes_follow_up_answer: boolean;
  includes_repo_evidence_text: boolean;
};

export type GitHubRepoInterviewMemoryCandidatePayload = {
  id?: string;
  type: string;
  value: string;
  label?: string;
  ...
  persisted: false;
  privacy: GitHubRepoInterviewMemoryPrivacy;
};

export type GitHubRepoInterviewMemory = Omit<
  GitHubRepoInterviewMemoryCandidatePayload,
  'requires_user_confirmation' | 'status' | 'persisted'
> & {
  id: string;
  type: string;
  value: string;
...
export type GitHubRepoInterviewVectorMemory = Omit<
  GitHubRepoInterviewVectorMemoryCandidate,
  | 'content_hash'
  | 'source_thread_id'
...
export type AgentRunDiagnostics = {
  agent_run: {
    ...
    status: 'running' | 'completed' | 'stopped' | 'failed' | 'error' | string;
  ...
  redis: {
    response_list_key: string;
    response_count: number;
    active_instance_keys: string[];
    control_channel: string;
    response_channel: string;
  };
};

export type AgentEvalMetricStatus = 'pass' | 'warn' | 'fail' | 'not_applicable';
export type AgentEvalOverallStatus = 'healthy' | 'attention' | 'failed';
...
export type AgentEvalTimelineItem = {
  time: string | null;
  kind: string;
...
export type AgentRunEvalReport = {
  run: AgentEvalRunSummary;
  overall: {
    status: AgentEvalOverallStatus;
    summary: string;
    next_steps: string[];
  };
  metrics: AgentEvalMetric[];
  token_usage: AgentRunTokenUsage;
  timeline: AgentEvalTimelineItem[];
  observations: {
    tool_event_count: number;
    workflow_payload_count: number;
...
async function getGitHubRepoInterviewMemoryHeaders(): Promise<Record<string, string>> {
  const supabase = createClient();
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.access_token) {
    throw new NoAccessTokenAvailableError();
...
function githubRepoInterviewMemoryUrl(path = ''): string {
  if (!API_URL) {
    throw new Error(
      'Backend URL is not configured. Set NEXT_PUBLIC_BACKEND_URL in your environment.',
    );
  }
  return `${API_URL}/github-repo-interview/memories${path}`;
}

function githubRepoInterviewVectorMemoryUrl(path = ''): string {
  if (!API_URL) {
    throw new Error(
      'Backend URL is not configured. Set NEXT_PUBLIC_BACKEND_URL in your environment.',
    );
  }
  return `${API_URL}/github-repo-interview/text-memories${path}`;
}

async function throwGitHubRepoInterviewMemoryError(
  response: Response,
  operation: string,
): Promise<never> {
  const rawBody = await response.text();
  let message = rawBody || response.statusText || 'Request failed';
  try {
    ...
    if (rawBody.trim().startsWith('<')) {
      message = response.statusText || 'Unexpected HTML error response';
    }
  }
  const error = new Error(
    `${operation} failed (${response.status}): ${message.slice(0, 240)}`,
...
export type GitHubRepoInterviewVectorMemoryApiError = Error & {
  status?: number;
  detail?: Record<string, unknown>;
};

async function throwGitHubRepoInterviewVectorMemoryError(
  response: Response,
  operation: string,
): Promise<never> {
  const rawBody = await response.text();
  ...
  try {
    const parsed = JSON.parse(rawBody);
    detail = parsed && typeof parsed.detail === 'object' ? parsed.detail : undefined;
  } catch {
    detail = undefined;
  }
  const code = typeof detail?.code === 'string' ? detail.code : 'request_failed';
  const error = new Error(
    `${operation} failed (${response.status}): ${code}`,
  ) as GitHubRepoInterviewVectorMemoryApiError;
  error.status = response.status;
  error.detail = detail;
  throw error;
}

export async function getGitHubRepoInterviewMemories(): Promise<GitHubRepoInterviewMemory[]> {
  const response = await fetch(githubRepoInterviewMemoryUrl(), {
    method: 'GET',
    headers: await getGitHubRepoInterviewMemoryHeaders(),
  });
  if (!response.ok) {
    await throwGitHubRepoInterviewMemoryError(response, 'load GitHub interview memories');
  }
  const payload = await response.json();
  return Array.isArray(payload.memories) ? payload.memories : [];
}

export async function confirmGitHubRepoInterviewMemory(
  candidate: GitHubRepoInterviewMemoryCandidatePayload,
): Promise<GitHubRepoInterviewMemory> {
  const response = await fetch(githubRepoInterviewMemoryUrl(), {
    method: 'POST',
    headers: await getGitHubRepoInterviewMemoryHeaders(),
  ...
  if (!response.ok) {
    await throwGitHubRepoInterviewMemoryError(response, 'confirm GitHub interview memory');
  }
  return await response.json();
}

export async function deleteGitHubRepoInterviewMemory(
  memoryId: string,
): Promise<GitHubRepoInterviewMemory> {
  const response = await fetch(githubRepoInterviewMemoryUrl(`/${memoryId}`), {
    method: 'DELETE',
    headers: await getGitHubRepoInterviewMemoryHeaders(),
  });
  if (!response.ok) {
    await throwGitHubRepoInterviewMemoryError(response, 'delete GitHub interview memory');
  }
  return await response.json();
}

export async function getGitHubRepoInterviewVectorMemories(): Promise<GitHubRepoInterviewVectorMemory[]> {
  const response = await fetch(githubRepoInterviewVectorMemoryUrl(), {
    method: 'GET',
    headers: await getGitHubRepoInterviewMemoryHeaders(),
  });
  if (!response.ok) {
    await throwGitHubRepoInterviewVectorMemoryError(response, 'load vector memories');
  }
  const payload = await response.json();
  return Array.isArray(payload.memories) ? payload.memories : [];
}

export async function confirmGitHubRepoInterviewVectorMemory(input: {
  candidate: GitHubRepoInterviewVectorMemoryCandidate;
  source_thread_id: string;
}): Promise<GitHubRepoInterviewVectorMemory> {
  const response = await fetch(githubRepoInterviewVectorMemoryUrl(), {
    method: 'POST',
    headers: await getGitHubRepoInterviewMemoryHeaders(),
  ...
  if (!response.ok) {
    await throwGitHubRepoInterviewVectorMemoryError(response, 'confirm vector memory');
  }
  return await response.json();
}

export async function deleteGitHubRepoInterviewVectorMemory(
  memoryId: string,
): Promise<GitHubRepoInterviewVectorMemory> {
  const response = await fetch(githubRepoInterviewVectorMemoryUrl(`/${memoryId}`), {
    method: 'DELETE',
    headers: await getGitHubRepoInterviewMemoryHeaders(),
  });
  if (!response.ok) {
    await throwGitHubRepoInterviewVectorMemoryError(response, 'delete vector memory');
  }
  return await response.json();
}
...
export const getAgentRunDiagnostics = async (
  agentRunId: string,
  ...
  try {
    ...
    const response = await fetch(
      `${API_URL}/developer/agent-runs/${agentRunId}`,
      {
        headers: {
          Authorization: `Bearer ${session.access_token}`,
        },
    ...
    if (!response.ok) {
      throw new Error(
        `Error getting agent run diagnostics: ${response.statusText} (${response.status})`,
      );
    }
    ...
    console.error('Failed to get agent run diagnostics:', error);
    handleApiError(error, {
      operation: 'load agent run diagnostics',
      resource: 'agent run diagnostics',
      silent: true,
    });
    throw error;
  }
...
export const getAgentEvalRuns = async (
  params: { limit?: number; status?: string } = {},
): Promise<{ runs: AgentEvalRunSummary[] }> => {
  try {
    const supabase = createClient();
    const {
      data: { session },
    ...
    const url = new URL(`${API_URL}/developer/agent-evals/runs`);
    ...
    if (!response.ok) {
      throw new Error(
        `Error getting agent eval runs: ${response.statusText} (${response.status})`,
      );
    }
    ...
    console.error('Failed to get agent eval runs:', error);
    handleApiError(error, {
      operation: 'load agent eval runs',
      resource: 'agent eval runs',
      silent: true,
    });
    throw error;
  }
...
export const getAgentRunEvalReport = async (
  agentRunId: string,
): Promise<AgentRunEvalReport> => {
  try {
    const supabase = createClient();
    const {
      data: { session },
    ...
    const response = await fetch(
      `${API_URL}/developer/agent-evals/runs/${agentRunId}`,
      {
        headers: {
          Authorization: `Bearer ${session.access_token}`,
        },
    ...
    if (!response.ok) {
      throw new Error(
        `Error getting agent run eval report: ${response.statusText} (${response.status})`,
      );
    }
    ...
    console.error('Failed to get agent run eval report:', error);
    handleApiError(error, {
      operation: 'load agent run eval report',
      resource: 'agent run eval report',
      silent: true,
    });
    throw error;
  }
    ...
    models: [
      {
        ...
        short_name: 'deepseek/deepseek-v4-flash',
