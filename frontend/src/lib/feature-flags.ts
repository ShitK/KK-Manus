...
class FeatureFlagHttpError extends Error {
  constructor(
    message: string,
    public readonly status: number,
  ) {
    super(message);
    this.name = 'FeatureFlagHttpError';
  }
}

const isAuthOrPermissionError = (error: unknown): error is FeatureFlagHttpError => {
  return error instanceof FeatureFlagHttpError && (error.status === 401 || error.status === 403);
};

const handleFeatureFlagResponseError = (message: string, status: number): void => {
  if (status === 401 || status === 403) {
    throw new FeatureFlagHttpError(`${message}: ${status}`, status);
  }
...
export class FeatureFlagManager {
  ...
  async isEnabled(flagName: string): Promise<boolean> {
    try {
      ...
      if (!response.ok) {
        handleFeatureFlagResponseError(`Failed to fetch feature flag ${flagName}`, response.status);
        return false;
      }
  ...
  async getFlagDetails(flagName: string): Promise<FeatureFlag | null> {
    try {
      ...
      if (!response.ok) {
        handleFeatureFlagResponseError(`Failed to fetch feature flag details for ${flagName}`, response.status);
        return null;
      }
  ...
  async getAllFlags(): Promise<Record<string, boolean>> {
    try {
      ...
      if (!response.ok) {
        handleFeatureFlagResponseError('Failed to fetch all feature flags', response.status);
        return {};
      }
...
const fetchFeatureFlag = async (flagName: string): Promise<boolean> => {
  try {
    ...
    console.warn(`Error fetching feature flag ${flagName}:`, error);
    return false;
  }
};

const fetchFeatureFlagDetails = async (flagName: string): Promise<FeatureFlag | null> => {
  try {
    const response = await fetch(`${API_URL}/feature-flags/${flagName}`, {
      method: 'GET',
      headers: {
    ...
    console.warn(`Error fetching feature flag details for ${flagName}:`, error);
    return null;
  }
};
