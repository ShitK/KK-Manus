...
class DBConnection:
        ...
        if cls._instance is None:
            with cls._lock:
                ...
                if cls._instance is None:
                    ...
                    cls._instance._initialize_lock = None
                    cls._instance._initialize_lock_loop = None
    ...
    def _get_initialize_lock(self) -> asyncio.Lock:
        ...
        with self._lock:
            lock_loop_is_stale = (
                self._initialize_lock_loop is not None
                and self._initialize_lock_loop.is_closed()
            )
            if (
                self._initialize_lock is None
                or self._initialize_lock_loop is not current_loop
                or lock_loop_is_stale
            ):
                self._initialize_lock = asyncio.Lock()
                self._initialize_lock_loop = current_loop
    ...
    async def _initialize_unlocked(self):
        """初始化数据库连接池；调用方必须持有初始化锁"""
        if self._initialized:
            current_loop = asyncio.get_running_loop()
            pool_is_usable = (
                self._pool is not None
                and not self._pool.is_closing()
            ...
            logger.warning("PostgreSQL connection pool is stale; reinitializing")
