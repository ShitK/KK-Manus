...
STREAM_DEDUPE_MIN_DUPLICATE_LENGTH = 12
...
class ResponseProcessor:
    ...
    @staticmethod
    def _coerce_text_chunk(value: Any) -> str:
        if value is None:
            return ""
    ...
    @staticmethod
    def _normalize_text_delta(accumulated_content: str, raw_chunk_content: Any) -> str:
        accumulated = ResponseProcessor._coerce_text_chunk(accumulated_content)
        raw_chunk = ResponseProcessor._coerce_text_chunk(raw_chunk_content)
        if not raw_chunk:
            return ""

        if accumulated and raw_chunk.startswith(accumulated):
            return raw_chunk[len(accumulated):]

        if accumulated and len(raw_chunk) >= STREAM_DEDUPE_MIN_DUPLICATE_LENGTH:
            if accumulated.endswith(raw_chunk) or accumulated.startswith(raw_chunk):
                return ""

        return raw_chunk

    @staticmethod
    def _guard_content_for_failed_grounded_tools(
        accumulated_content: str,
        tool_completed_buffer: List[Dict[str, Any]],
    ) -> str:
        # Scope this guard to the initial repo-prep tool: it is the step that
        # creates the grounded question pack from fresh GitHub reads.
        github_repo_results: List[ToolResult] = []

        for tool_completion in tool_completed_buffer or []:
            context = tool_completion.get("context")
            if not context:
                continue
            tool_call = getattr(context, "tool_call", {}) or {}
            if tool_call.get("function_name") != "github_repo_interview_prep":
                continue

            result = getattr(context, "result", None)
            github_repo_results.append(result)

        for result in github_repo_results:
            if not getattr(result, "success", False):
                continue
            payload = safe_json_parse(getattr(result, "output", ""))
            # Tool execution success is not enough if the structured payload
            # still declares the grounded repo-prep result as failed. A partial
            # result is acceptable only when it already contains grounded
            # interview questions; file-read warnings should not wipe the demo.
            if isinstance(payload, dict) and payload.get("status") == "error":
                continue
            if (
                isinstance(payload, dict)
                and payload.get("status") == "partial"
                and not ResponseProcessor._github_repo_prep_has_grounded_questions(payload)
            ):
                continue
        ...
        for result in github_repo_results:
            ...
            if not isinstance(payload, dict):
                payload = {}
            if payload.get("status") not in {"error", "partial"} and payload:
                continue

            errors = ensure_list(payload.get("errors"), [])
            first_error = errors[0] if errors else {}
            code = first_error.get("code") or "github_repo_interview_prep_failed"
            message = first_error.get("message") or "GitHub repository evidence could not be loaded."
            language = ensure_dict(payload.get("input"), {}).get("language")

            if language == "en":
                return (
                    "GitHub repository reading failed, so I cannot provide grounded interview "
                    f"questions for this repository yet.\n\nError: {code} - {message}\n\n"
                    "Please retry after the GitHub request succeeds."
                )
            return (
                "GitHub 仓库读取失败，所以这次不能提供基于仓库证据的面试题。\n\n"
                f"错误：{code} - {message}\n\n"
                "请在 GitHub 请求成功后重试。"
            )
        return accumulated_content
    ...
    @staticmethod
    def _github_repo_prep_has_grounded_questions(payload: Dict[str, Any]) -> bool:
        data = ensure_dict(payload.get("data"), {})
        questions_payload = data.get("interview_questions")
        if isinstance(questions_payload, dict):
            questions_payload = questions_payload.get("questions")
        questions = ensure_list(questions_payload, [])
        if not questions:
            return False

        summary = ensure_dict(data.get("question_generation_summary"), {})
        if summary and not (
            summary.get("has_source_evidence") or summary.get("has_evidence_details")
        ):
            return False
        ...
        for question in questions:
            if not isinstance(question, dict):
                continue
            evidence_details = ensure_list(question.get("evidence_details"), [])
            has_readable_detail = any(
                isinstance(detail, dict) and detail.get("source_path")
                for detail in evidence_details
            )
            if has_readable_detail:
                return True
            if ensure_list(question.get("evidence_refs"), []) and ensure_list(
                question.get("source_paths"), []
            ):
                return True
        return False
    ...
    async def process_adk_streaming_response(
        ...
        try:
                    ...
                    if parts:            
                        for delta in parts:
                            ...
                            if hasattr(delta, 'text') and delta.text:
                                raw_chunk_content = delta.text
                                chunk_content = self._normalize_text_delta(
                                    accumulated_content,
                                    raw_chunk_content,
                                )

                                if not chunk_content:
                                    logger.info("Skipping duplicate or empty ADK text chunk")
                                    continue
                                ...
                                logger.info(f"current accumulated_content before append: {accumulated_content}")
            ...
            if accumulated_content and not should_auto_continue:
                guarded_content = self._guard_content_for_failed_grounded_tools(
                    accumulated_content,
                ...
                if last_assistant_message_object:
                    try:
                        ...
                        await self.add_message(
                            ...
                            metadata={
                                "thread_run_id": thread_run_id,
                                **({"agent_run_id": config.agent_run_id} if config.agent_run_id else {}),
                            }
                        )
